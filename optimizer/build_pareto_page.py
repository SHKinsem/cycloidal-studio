# -*- coding: utf-8 -*-
"""从 results/pareto_front.csv 生成自包含的帕累托前沿浏览器 (写到仓库根 pareto.html)。
运行: python build_pareto_page.py"""
import csv, os

HERE = os.path.dirname(os.path.abspath(__file__))
CSV_IN = os.path.join(HERE, "results", "pareto_front.csv")
HTML_OUT = os.path.join(HERE, "..", "pareto.html")
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  canvas{display:block;width:100%;max-width:100%;min-width:0;aspect-ratio:4/3}
...
with open(HTML_OUT, "w", encoding="utf-8", newline="\n") as f:
    f.write(html)
print(f"[OK] pareto.html written ({len(rows)} designs, {len(html)} bytes)")
